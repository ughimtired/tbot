...
class Menu:
    ...
    def loadCurMenu(self):
        if os.path.getsize(self.namePickleFile) > 0:
...
m_games = Menu("Игры", buttons=["Камень, ножницы, бумага", "КНБ Multiplayer", "Игра больше-меньше", "Игра в 21", "Угадай кто?", "Выход"], parent=m_main)
...
m_game_lm = Menu("Игра больше-меньше", buttons=["Больше", "Меньше", "Равно", "Выход"], parent=m_games, handler="gameML")
m_DZ = Menu("ДЗ", buttons=["Задание-1", "Задание-2", "Задание-3", "Задание-4", "Задание-5", "Задание-6", "Выход"], parent=m_main)

m_fun = Menu("Развлечения", buttons=["Прислать собаку", "Прислать лису", "Прислать анекдот", "Прислать палитру",  "Прислать новости", "Прислать фильм", "Выход"], parent=m_main)
Menu.loadCurMenu()